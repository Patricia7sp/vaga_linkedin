...
def create_databricks_connect_session():
    ...
    try:
        ...
        SparkSession, col, current_timestamp, StructType, StructField, StringType, IntegerType, BooleanType = _import_spark()
        ...
        try:
            ...
            spark = SparkSession.builder \
                .appName("VagaLinkedInLoadAgent-Connect") \
...
def create_databricks_session_legacy():
    ...
    try:
        ...
        spark = SparkSession.builder \
            .appName("VagaLinkedInLoadAgent-Legacy") \
...
def create_spark_session_local():
    ...
    try:
        ...
        try:
            ...
            spark = SparkSession.builder \
                .appName("VagaLinkedInLoadAgent-Local") \
                .config("spark.sql.adaptive.enabled", "true") \
                .config("spark.sql.adaptive.coalescePartitions.enabled", "true") \
                .config("spark.serializer", "org.apache.spark.serializer.KryoSerializer") \
...
def create_catalog_via_api():
    ...
    try:
        ...
        catalog_data = {
            "name": CATALOG,
            "comment": "Catálogo para dados de vagas do LinkedIn"
        }
    ...
    try:
        ...
        try:
            result = subprocess.run([
                ...
                "--name", CATALOG,
                "--comment", f"Catálogo para pipeline Vagas LinkedIn - {datetime.now().strftime('%Y-%m-%d')}"
            ...
            for layer in LAYERS:
                ...
                try:
                    result = subprocess.run([
                        ...
                        "--name", full_schema,
                        "--comment", f"Schema {layer} para domínio {domain} - pipeline LinkedIn"
