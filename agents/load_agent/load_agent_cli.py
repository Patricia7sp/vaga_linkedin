...
def create_storage_credentials():
    ...
    try:
        ...
        result = subprocess.run([
            ...
            "--gcp-sak-email", gcs_creds["client_email"],
            "--gcp-sak-private-key-id", gcs_creds["private_key_id"],
            "--gcp-sak-private-key", gcs_creds["private_key"],
            "--comment", "GCS credentials para vagas LinkedIn pipeline"
...
def create_external_location(storage_cred_name):
    ...
    try:
        ...
        result = subprocess.run([
            ...
            "--url", gcs_uri,
            "--storage-credential-name", storage_cred_name,
            "--comment", "External location para bucket GCS do pipeline LinkedIn"
        ], capture_output=True, text=True, timeout=30)
...
def create_catalog_with_external_location(catalog_name, external_location_name):
    ...
    try:
        ...
        result = subprocess.run([
            ...
            "--name", catalog_name,
            "--comment", f"Catálogo vagas LinkedIn com external location - {datetime.now().strftime('%Y-%m-%d')}"
        ], capture_output=True, text=True, timeout=30)
...
def create_unity_catalog_via_cli():
    ...
    try:
        ...
        try:
            result = subprocess.run([
                "databricks", "unity-catalog", "catalogs", "get",
                "--name", CATALOG
            ...
            for layer in LAYERS:
                ...
                try:
                    result = subprocess.run([
                        ...
                        "--name", schema_name,
                        "--comment", f"Schema {layer} para domínio {domain}"
                    ], capture_output=True, text=True, timeout=15)
    ...
    try:
        ...
        for domain in DOMAINS:
            ...
            cmd = [
                ...
                CATALOG, schema_name, volume_name, "EXTERNAL",
                "--storage-location", gcs_location,
                "--comment", f"Volume para dados JSON {domain.replace('_', ' ').title()} do LinkedIn no GCS"
            ]
    ...
    try:
        ...
        if volumes_created:
            ...
            if tables_created:
                ...
                results["summary"] = f"Unity Catalog: {results['schemas_created']} schemas, {len(volumes_created)} volumes, {len(tables_created)} tabelas RAW, {total_records} registros"
            else:
                print("⚠️  Tabelas não criadas - usando fallback metadata")
                table_metadata = create_table_metadata()
                results["table_metadata"] = table_metadata
                results["status"] = "volumes_only"
                results["summary"] = f"Unity Catalog: {results['schemas_created']} schemas, {len(volumes_created)} volumes, metadados apenas, {total_records} registros"
        else:
            ...
            if creation_results["total_success"] > 0:
                print(f"✅ Fallback: {len(creation_results['volumes_created'])} volumes + {len(creation_results['tables_created'])} tabelas via SQL")
                table_metadata = create_table_metadata()
                results["table_metadata"] = table_metadata
                results["status"] = "ready_for_transform_agent"
                results["summary"] = f"Unity Catalog: {results['schemas_created']} schemas, {len(creation_results['volumes_created'])} volumes, {len(creation_results['tables_created'])} tabelas RAW, {total_records} registros"
            else:
                print("⚠️  Erro em volumes e tabelas - usando fallback metadata")
            table_metadata = create_table_metadata()
            results["table_metadata"] = table_metadata
            results["status"] = "metadata_only"
            results["summary"] = f"Unity Catalog: {results['schemas_created']} schemas, metadados apenas, {total_records} registros"
