...
def run_transform(instructions=None):
    ...
    try:
        ...
        df['tecnologia_principal'] = df['skills'].apply(
            lambda x: x[0] if isinstance(x, list) and len(x) > 0 else 'N/A'
        )
        
        # Extract salary numbers
        df['salario_min'] = df['salary_range'].str.extract(r'R\$ ([\d,]+)').iloc[:, 0].str.replace(',', '').astype(float, errors='ignore')
        df['salario_max'] = df['salary_range'].str.extract(r'- R\$ ([\d,]+)').iloc[:, 0].str.replace(',', '').astype(float, errors='ignore')
        
        # Create experience level from title
        df['nivel_experiencia'] = df['title'].apply(lambda x: 
            'Senior' if 'Sênior' in x or 'Senior' in x else
            'Pleno' if 'Pleno' in x else
            'Junior' if 'Junior' in x or 'Jr' in x else
            'Não especificado'
        )
        ...
        # Main jobs table
        jobs_table = df_clean[[
            'id', 'title', 'company', 'cidade', 'estado', 'posted_date',
            'description', 'salary_range', 'salario_min', 'salario_max',
            'nivel_experiencia', 'tecnologia_principal'
        ]]
        ...
        # Companies summary
        companies_df = df_clean.groupby('company').agg({
            'id': 'count',
            'salario_min': 'mean',
            'salario_max': 'mean'
        }).rename(columns={'id': 'total_vagas'}).reset_index()
