    ...
    if RAPIDAPI_AVAILABLE:
        try:
            ...
            jobs = extract_jobs_via_rapidapi(
                search_term=search_term, location=location, max_results=max_results, category=category
            )
...
def run_hybrid_extraction(categories: Dict[str, List[str]] = None, output_dir: str = "./data/linkedin_jobs") -> Dict:
    """
    ...
    results = {}
    stats = {"rapidapi_count": 0, "selenium_count": 0, "total": 0}
    ...
    for category, search_terms in categories.items():
        ...
        for search_term in search_terms:
            jobs = extract_jobs_hybrid(search_term=search_term, location="Brazil", max_results=50, category=category)
