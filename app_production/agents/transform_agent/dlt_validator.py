...
class DLTValidator:
    ...
    def __init__(self):
        self.known_columns = {
            ...
            'category', 'extract_date', 'work_modality', 'contract_type',
            'salary_min', 'salary_max', 'has_salary', 'posted_date',
            'posted_time', 'search_term', 'source', 'url'
        }
        ...
        try:
                ...
                if usage in content:
                    ...
                    if not is_imported:
                        validation_result['errors'].append(
                            f"Function '{func_name}' is used but not imported"
                        )
            ...
            for invalid_col in self.invalid_columns:
                if f'"{invalid_col}"' in content or f"'{invalid_col}'" in content:
                    validation_result['errors'].append(
                        f"Reference to non-existent column '{invalid_col}'"
                    )
                    validation_result['valid'] = False
            ...
            if 'dlt.read_stream("' + domain + '_raw")' in content:
                validation_result['errors'].append(
                    f"Cannot use dlt.read_stream on external schema '{domain}_raw'"
                )
                validation_result['auto_fixes'].append(
                    "Use spark.readStream with cloudFiles for external data"
                )
                validation_result['valid'] = False
            ...
            for vol in volumes:
                ...
                if vol != expected:
                    validation_result['errors'].append(
                        f"Wrong volume path: {vol}, expected {expected}"
                    )
                    validation_result['valid'] = False
    ...
    def auto_correct_notebook(self, notebook_path: str, domain: str) -> bool:
        ...
        try:
            ...
            # Fix table names
            content = re.sub(
                r'name\s*=\s*["\']data_analytics_gold["\']',
                'name="gold_data_analytics"',
                content
            )
            content = re.sub(
                r'name\s*=\s*["\']data_engineer_gold["\']',
                'name="gold_data_engineer"',
                content
            )
            content = re.sub(
                r'name\s*=\s*["\']digital_analytics_gold["\']',
                'name="gold_digital_analytics"',
                content
            )
