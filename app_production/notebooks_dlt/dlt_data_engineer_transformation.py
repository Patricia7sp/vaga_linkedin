...
# =========================
# Volume UC com JSONs organizados por dia: /YYYY-MM-DD/data_engineer_*.json|jsonl
VOLUME_ROOT = "/Volumes/vagas_linkedin/data_engineer_raw/linkedin_data_volume"
# Schema location para Auto Loader
SCHEMA_LOC = "/Volumes/vagas_linkedin/data_engineer_raw/linkedin_data_volume/_schemas/bronze_linkedin_v10"
...
def bronze_data_engineer():
    ...
    df = (
        spark.readStream
            ...
            .load(VOLUME_ROOT)
            .where(col("_metadata.file_name").rlike("(?i)^data_engineer_.*\\.json(l)?$"))
            ...
            .withColumn(
                ...
                expr("""
                  coalesce(
                    regexp_extract(_metadata.file_path, '.*/(\\d{4}-\\d{2}-\\d{2})/.*', 1),
    ...
    return (
        dlt.read_stream("data_engineer_silver_stg")
          ...
          .select(
              ...
              "url", "extract_date", "ingestion_timestamp", "processed_timestamp", "source_file"
          )
    )
