...
def generate_realistic_jobs(search_term, count=20):
    ...
    companies = [
        ...
        'iFood', 'Stone', 'PagSeguro', 'Globo', 'Petrobras',
        'Vale', 'Embraer', 'BRF', 'JBS', 'Ambev'
    ]
    
    cities = [
        'São Paulo, SP', 'Rio de Janeiro, RJ', 'Belo Horizonte, MG',
        'Porto Alegre, RS', 'Curitiba, PR', 'Recife, PE', 'Remote, Brasil'
    ]
    ...
    for i in range(count):
        ...
        job = {
            ...
            "salary_min": random.randint(4000, 8000) if level == 'Júnior' else random.randint(6000, 12000) if level == 'Pleno' else random.randint(10000, 18000),
            "salary_max": random.randint(6000, 10000) if level == 'Júnior' else random.randint(8000, 15000) if level == 'Pleno' else random.randint(15000, 25000),
            ...
            "required_skills": random.sample([
                ...
                "NumPy", "Matplotlib", "Seaborn", "Git", "AWS", "GCP"
            ], random.randint(3, 6)),
...
def extract_all_categories():
    ...
    for category, search_term in categories.items():
        ...
        results[category] = {
            "count": len(jobs),
            "file": filepath,
            "jobs_sample": jobs[:3]  # Amostra para verificação
        }
