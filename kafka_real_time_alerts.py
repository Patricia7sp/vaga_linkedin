...
def send_new_job_alert(producer, job, category):
    ...
    try:
        ...
        alert_payload = {
            ...
            "priority": (
                "HIGH"
                if any(word in job.get("title", "").lower() for word in ["senior", "sênior", "lead"])
                else "MEDIUM"
            ),
        }
            ...
            for category, search_terms in categories.items():
                ...
                for search_term in search_terms:
                    jobs = extract_jobs_via_linkedin_scraping(
                        search_term, max_results=10, category=category  # Menos vagas para monitoramento rápido
                    )
                    if jobs:
...
def run_single_check():
    ...
    categories = {"data_engineer": ["Data Engineer"], "data_analytics": ["Data Analytics"]}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--interval", type=int, default=15, help="Intervalo de verificação em minutos (para modo monitor)"
    )
